...
uses
  ...
  Vcl.ComCtrls, Vcl.Shell.ShellCtrls, Vcl.ExtDlgs, Vcl.StdCtrls, Vcl.Buttons,
  Vcl.ExtCtrls;
...
begin
  if OpenPictureDialog1.execute then
    i10.picture.loadfromfile(OpenPictureDialog1.filename);
end;

procedure TForm1.S10C(Sender: TObject);
begin
  if OpenPictureDialog1.execute then
    i8.picture.loadfromfile(OpenPictureDialog1.filename);
end;

procedure TForm1.S14C(Sender: TObject);
begin
  if SaveDialog2.execute then
    i10.picture.savetofile(SaveDialog2.filename);
end;

procedure TForm1.S12C(Sender: TObject);
begin
  if SaveDialog2.execute then
    i8.picture.savetofile(SaveDialog2.filename);
end;

procedure TForm1.S15Click(Sender: TObject);
var dd:file of byte;
    anz:integer;
begin
...
procedure TForm1.S16Click(Sender: TObject);
begin
  if savedialog1.execute then m2.lines.savetofile(savedialog1.filename);
end;

procedure TForm1.CKodieren(Sender: TObject);
label 1;
var
    x, y, i, j, z: Integer;
    PixelData: TColor;
...
procedure TForm1.CDekodieren(Sender: TObject);
label 1;
Var
  x, y, z: integer;
  mask, ch: byte;
...
procedure TForm1.D22C(Sender: TObject);
begin
  suchabbruch:=true;
end;

procedure TForm1.CB5C(Sender: TObject);
begin
  imgdelta.visible:=check1.checked;
end;

procedure TForm1.S17Click(Sender: TObject);
begin
